      ...
      {orders.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          <input
            ...
            className="border border-gray-200 rounded px-3 py-2 text-sm outline-none focus:border-gray-400 transition-colors flex-1 min-w-48"
          />
          <select
            value={statusFilter}
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-sm text-gray-400">
                  {query || statusFilter ? "No orders match your filter." : "No orders yet. Orders will appear here once customers check out."}
                </td>
              </tr>
            ) : (
              filtered.map((order) => (
