...
export default function CouponsClient({ shopId, currency, coupons: initial }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (form.type === "percentage" && val > 100) return toast.error("Percentage cannot exceed 100");
  ...
  return (
    <>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="border border-gray-200 rounded-lg p-5 flex flex-col gap-4 bg-gray-50">
          <p className="text-sm font-medium text-gray-900">New coupon</p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700">Code <span className="text-red-400">*</span></label>
              <input
                value={form.code}
                onChange={(e) => patch("code", e.target.value.toUpperCase())}
                placeholder="SUMMER20"
                className="border border-gray-200 rounded px-3 py-2 text-sm font-mono outline-none focus:border-gray-400 transition-colors bg-white"
              />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700">Type <span className="text-red-400">*</span></label>
              <select
                value={form.type}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700">
                ...
                <span className="text-gray-400 font-normal ml-1">{form.type === "percentage" ? "%" : currency}</span>
              </label>
              <input
                type="number"
                min="0.01"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700">
                Min order <span className="text-gray-400 font-normal">({currency}, optional)</span>
              </label>
              <input
                type="number"
                min="0"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700">Max uses <span className="text-gray-400 font-normal">(optional)</span></label>
              <input
                type="number"
                min="1"
                step="1"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700">Expires <span className="text-gray-400 font-normal">(optional)</span></label>
              <input
                type="date"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-left">
              <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Code</th>
              <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Discount</th>
              <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Uses</th>
              <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Expires</th>
              ...
              return (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-500 text-xs">
                    {c.expiresAt ? (
                      <span className={expired ? "text-red-500" : ""}>{formatDate(c.expiresAt)}</span>
                    ) : "Never"}
                  </td>
                  <td className="px-4 py-3">
                    {expired || exhausted ? (
                      ...
                      >
                        {togglingId === c.id ? "…" : effectivelyActive ? "Active" : "Inactive"}
                      </button>
                    )}
                  </td>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-6 flex flex-col gap-5">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                ...
                will be permanently removed and can no longer be used at checkout.
              </p>
            </div>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setConfirmDeleteId(null)} className="px-3 py-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors">
                Cancel
              </button>
              <button onClick={handleDelete} className="px-4 py-1.5 bg-red-500 text-white text-sm font-medium rounded hover:bg-red-600 transition-colors">
                Delete
              </button>
            </div>
          </div>
