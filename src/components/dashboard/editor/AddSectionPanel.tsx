...
const sectionIcons: Record<AddableSectionType, React.ReactNode> = {
  announcement: (
    <svg
      className="w-4 h-4"
      fill="none"
  ...
  "image-text": (
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M3.75 6.75h16.5M3.75 12h16.5M3.75 17.25h7.5"
      />
    </svg>
  ),
  gallery: (
...
export default function AddSectionPanel({ onAdd, onAddNavbar, hasNavbar, onClose, themeId, currentPage }: Props) {
  const visibleSections = addableSections.filter((s) => {
    if (s.themes && !s.themes.includes(themeId)) return false;
    if (s.pages && !s.pages.includes(currentPage)) return false;
    return true;
  });
  ...
  return (
    <div className="border-t border-neutral-100 bg-neutral-50 overflow-scroll">
      ...
      <ul className="py-1 max-h-80 overflow-y-auto">
        ...
        {visibleSections.map(({ type, description }) => (
          <li key={type}>
            <button
              onClick={() => onAdd(type)}
              className="w-full flex items-start gap-3 px-4 py-2.5 hover:bg-white transition-colors text-left"
